...
function _remapEnlink(raw) {
  var out = {};
  if (raw.batt_mv !== undefined) out.battery_voltage = raw.batt_mv / 1000;
  if (raw.batt_status !== undefined) {
    var s = raw.batt_status;
    out.battery_status = s === 2 ? 'Low' : s === 1 ? 'Good' : 'Unknown';
  }
  if (raw.fan_run_time_s !== undefined) out.fan_runtime = raw.fan_run_time_s;
  if (raw.temperature_c !== undefined) out.temperature = raw.temperature_c;
  if (raw.humidity !== undefined)       out.humidity = raw.humidity;
  if (raw.lux !== undefined)            out.light_level = raw.lux;
  if (raw.pressure_mbar !== undefined)  out.barometric_pressure = raw.pressure_mbar;
  if (raw.iaq !== undefined)            out.indoor_air_quality_voc = raw.iaq;
  if (raw.bvoc !== undefined)           out.breath_voc = raw.bvoc;
  if (raw.co2e_ppm !== undefined)       out.co2e = raw.co2e_ppm;
  if (raw.co2_ppm !== undefined)        out.co2 = raw.co2_ppm;
  if (raw.det_count !== undefined)      out.motion_detection_count = raw.det_count;
  if (raw.occ_time_s !== undefined)     out.motion_occupied_time = raw.occ_time_s;
  if (raw.sound_min_dba !== undefined)  out.sound_min = raw.sound_min_dba;
  if (raw.sound_avg_dba !== undefined)  out.sound_avg = raw.sound_avg_dba;
  if (raw.sound_max_dba !== undefined)  out.sound_max = raw.sound_max_dba;
  if (raw.dp_pa !== undefined)          out.differential_pressure = raw.dp_pa;
  if (raw.af_mps !== undefined)         out.air_flow = raw.af_mps;
  if (raw.gp_pa !== undefined)          out.gauge_pressure = raw.gp_pa;
  if (raw.gp_t_c !== undefined)         out.gauge_sensor_temperature = raw.gp_t_c;
  if (raw.pulse_ip1 !== undefined)      out.pulse_counter1 = raw.pulse_ip1;
  if (raw.pulse_ip2 !== undefined)      out.pulse_counter2 = raw.pulse_ip2;
  if (raw.pulse_ip3 !== undefined)      out.pulse_counter3 = raw.pulse_ip3;
  if (raw.adc_kohm !== undefined)       out.adc_kohm = raw.adc_kohm;
  if (raw.leak_detect_event !== undefined) out.leak_detected = raw.leak_detect_event;
  // particles (mass concentration µg/m³)
  if (raw.mc_pm0_1 !== undefined)  out.particles_pm0_1_mass_concentration  = raw.mc_pm0_1;
  if (raw.mc_pm0_3 !== undefined)  out.particles_pm0_3_mass_concentration  = raw.mc_pm0_3;
  if (raw.mc_pm1_0 !== undefined)  out.particles_pm1_mass_concentration    = raw.mc_pm1_0;
  if (raw.mc_pm2_5 !== undefined)  out.particles_pm2_5_mass_concentration  = raw.mc_pm2_5;
  if (raw.mc_pm4_0 !== undefined)  out.particles_pm4_mass_concentration    = raw.mc_pm4_0;
  if (raw.mc_pm5_0 !== undefined)  out.particles_pm5_mass_concentration    = raw.mc_pm5_0;
  if (raw.mc_pm10_0 !== undefined) out.particles_pm10_mass_concentration   = raw.mc_pm10_0;
  // gas
  if (raw.o2perc !== undefined)    out.o2 = raw.o2perc;
  if (raw.co_ppm !== undefined)    out.co = raw.co_ppm;
  if (raw.ozone_ppm !== undefined) out.ozone_ppm = raw.ozone_ppm;
  if (raw.ozone_ppb !== undefined) out.ozone_ppb = raw.ozone_ppb;
  if (raw.pollutants_kohm !== undefined) out.pollutants = raw.pollutants_kohm;
  if (raw.h2s_ppm !== undefined)   out.h2s_ppm = raw.h2s_ppm;
  if (raw.no_ppm !== undefined)    out.no_ppm = raw.no_ppm;
  if (raw.no2_ppm !== undefined)   out.no2_ppm = raw.no2_ppm;
  if (raw.so2_ppm !== undefined)   out.so2 = raw.so2_ppm;
  // modbus (1..32)
  for (var n = 1; n <= 32; n++) {
    var pad = n < 10 ? '0' + n : '' + n;
    var ex  = raw['mb_' + pad + '_ex'];
    var iv  = raw['mb_' + pad + '_int'];
    var cv  = raw['mb_' + pad + '_cum'];
    if (ex  !== undefined) out['modbus_' + n + '_exception']   = ex;
    if (iv  !== undefined) out['modbus_' + n + '_interval']    = iv;
    if (cv  !== undefined) out['modbus_' + n + '_cumulative']  = cv;
  }
  return out;
}

function decodeUplink(input) { return { data: _remapEnlink(enlinkDecode(input.bytes)) }; }
function Decode(fPort, bytes) { return _remapEnlink(enlinkDecode(bytes)); }
function Decoder(bytes, port) { return _remapEnlink(enlinkDecode(bytes)); }
