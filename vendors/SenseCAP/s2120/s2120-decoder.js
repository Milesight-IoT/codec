...
function decodeUplink(input) {
  ...
  var result = _scDecode(input);
  var srcData = (result && result.data) ? result.data : {};
  var out = { raw_uplink: raw };

  var messages = srcData.messages || [];
  for (var i = 0; i < messages.length; i++) {
    var msg = messages[i];
    if (msg.type === 'upload_battery' && msg.battery !== undefined) {
      out.battery_percentage = msg.battery;
    } else if (msg.type === 'report_telemetry' && msg.measurementId !== undefined) {
      var fieldId = _MEAS_MAP[msg.measurementId];
      if (fieldId) out[fieldId] = msg.measurementValue;
    }
  }
  // s2100 datalogger: also map from dynamic messages by measurementId
  return { data: out };
}
